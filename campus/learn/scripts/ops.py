...
from _config import get_download_dir_abs, get_upload_dir, get_semester, \
    get_student_id, get_student_name, get_submissions_log
...
def ensure_api():
    ...
    if not api.reload_session():
        print(json.dumps({"status": "error", "message": "Session 无效，请先运行 base-cas login"}, ensure_ascii=False))
        raise SystemExit(1)
    return api
...
def download_file(wlkcid, wjid, filename=None, save_dir=None):
    ...
    if path:
        ...
        print(json.dumps({"status": "error", "message": "下载失败"}, ensure_ascii=False))
    return path
...
def check_graded_submissions(api):
    ...
    注意：zyListYpg 端点返回的 zt 字段为"已交"而非"已批改"，
    因此按端点来源判断：出现在 zyListYpg 中的即为已批改。
    """
    log = _read_log()
    if not log:
        return []
    ...
    # 按课程分组查询 zyListYpg，避免逐条重复请求
    courses_seen = {}
    for xszyid, entry in log.items():
        wlkcid = entry.get("wlkcid", "")
        if wlkcid not in courses_seen:
            data = {"aoData": [{"name": "wlkcid", "value": wlkcid}]}
            try:
                d = api._post("/b/wlxt/kczy/zy/student/zyListYpg", data, use_ajax=True)
                courses_seen[wlkcid] = {h.get("xszyid", ""): h for h in d.get("object", {}).get("aaData", [])}
            except Exception:
                courses_seen[wlkcid] = {}

    for xszyid, entry in log.items():
        wlkcid = entry.get("wlkcid", "")
        graded_map = courses_seen.get(wlkcid, {})
        if xszyid in graded_map:
            h = graded_map[xszyid]
            # zyListYpg API 直接返回 cj(成绩)、pynr(评语)、pysjStr(批改时间)
            score_val = h.get("cj", "")
            comment_val = (h.get("pynr", "") or "")[:500]
            graded.append({
                "course": entry.get("course", ""),
                "title": entry.get("title", ""),
                "score": str(score_val) if score_val != "" else "",
                "comment": comment_val,
                "graded_at": h.get("pysjStr", ""),
            })
...
def move_to_workspace(source_path):
    ...
    if not os.path.exists(source_path):
        print(json.dumps({"status": "error", "message": f"源文件不存在: {source_path}"}, ensure_ascii=False))
        return None
...
if __name__ == '__main__':
    ...
    if args.action == 'list-files':
        if not args.course:
            print(json.dumps({"status": "error", "message": "需要 --course"}, ensure_ascii=False)); sys.exit(1)
        ...
        if not args.course or not args.wjid:
            print(json.dumps({"status": "error", "message": "需要 --course 和 --wjid"}, ensure_ascii=False)); sys.exit(1)
        ...
        if not args.course or not args.xszyid or not args.file:
            print(json.dumps({"status": "error", "message": "需要 --course --xszyid --file"}, ensure_ascii=False)); sys.exit(1)
        if not args.confirm:
            print(json.dumps({
                "status": "pending",
        ...
        if not args.file:
            print(json.dumps({"status": "error", "message": "需要 --file"}, ensure_ascii=False)); sys.exit(1)
        ...
        if not args.input_dir:
            print(json.dumps({"status": "error", "message": "需要 --input-dir"}, ensure_ascii=False)); sys.exit(1)
